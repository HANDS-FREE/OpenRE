...
void HF_Flash_Read(u32 ReadAddr,u16 *pBuffer,u16 NumToRead)
{
    ...
    {
        pBuffer[i]=STMFLASH_ReadWord(ReadAddr);//read 2 bytes
...
void HF_Flash_Write(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)
{
    ...
    while(1) 
    {	
        HF_Flash_Read(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE,STMFLASH_BUF,STM_SECTOR_SIZE/2);//read all data from sector
