...
void LCD_delay_us(unsigned short t)
{
    delay_us(t);
}

void LCD_delay_ms(unsigned short t)
{
    delay_ms(t);
}
...
#define LCD_ILI9341_CMD(index)       ((*(__IO u16 *) (Bank1_LCD_C)) = ((u16)index))
#define LCD_ILI9341_Parameter(val)	 ((*(__IO u16 *) (Bank1_LCD_D)) = ((u16)(val)))

//写寄存器函数
//regval:寄存器值
void LCD_WR_REG(unsigned short int regval)
{   
    LCD_ILI9341_CMD(regval);
}
...
//读LCD数据
//返回值:读到的值
unsigned short int LCD_RD_DATA(void)
{
...
{
    unsigned short int regval=0;
    unsigned short int dirreg=0;
...
//x,y:坐标
//color:颜色
void LCD_Fast_DrawPoint(unsigned short int x,unsigned short int y,unsigned short int color)
{	   
    if(lcddev.id==0X9341||lcddev.id==0X5310)
    {
        LCD_WR_REG(lcddev.setxcmd);
...
{    
    unsigned char hsareg,heareg,vsareg,veareg;
    unsigned short int hsaval,heaval,vsaval,veaval;
    unsigned short int twidth,theight;
    twidth=sx+width-1;
    theight=sy+height-1;
...
void LCD_GPIO_Config(void)
{
    ...
    RCC_APB2PeriphClockCmd( RCC_APB2Periph_GPIOD | RCC_APB2Periph_GPIOE | RCC_APB2Periph_GPIOB,ENABLE); /* 使能FSMC对应相应管脚时钟*/
    ...
    //PD7-FSMC_NE1   :LCD-CS  片选信号
    //PD11-FSMC_A16  :LCD-DC  写指令和写数据
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode =  GPIO_Mode_AF_PP;
...
{ 					
    ...
    else if(lcddev.id==0X9331)//OK |/|/|
    {
        ...
        LCD_delay_ms(200); // Dis-charge capacitor power voltage
...
//color:要填充的颜色
void LCD_Fill(unsigned short int sx,unsigned short int sy,unsigned short int ex,unsigned short int ey,unsigned short int color)
{          
    unsigned short int i,j;
    unsigned short int xlen=0;
    unsigned short int temp;
    if((lcddev.id==0X6804)&&(lcddev.dir==1))	//6804横屏的时候特殊处理
    {
        temp=sx;
...
//(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)   
//color:要填充的颜色
void LCD_Color_Fill(unsigned short int sx,unsigned short int sy,unsigned short int ex,unsigned short int ey,unsigned short int *color)
{  
...
//m^n函数
//返回值:m^n次方.
u32 LCD_Pow(unsigned char m,unsigned char n)
{
    u32 result=1;
    while(n--)result*=m;
    return result;
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(unsigned short int x,unsigned short int y,u32 num,unsigned char len,unsigned char size)
{         	
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(unsigned short int x,unsigned short int y,u32 num,unsigned char len,unsigned char size,unsigned char mode)
{  
    unsigned char t,temp;
    unsigned char enshow=0;
    for(t=0;t<len;t++)
