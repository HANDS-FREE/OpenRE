	...
	else
	{
	  GPIO_InitTypeDef  GPIO_InitStructure;
		...
		//所以上拉之前,必须使能时钟.才能实现真正的上拉输出
	 	...
	 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOD, ENABLE);	 //使能PB,PF端口时钟
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;				     // PB1端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	 	GPIO_Init(GPIOE, &GPIO_InitStructure); //B1推挽输出
	 	GPIO_SetBits(GPIOE,GPIO_Pin_0);        //上拉
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;				   // PB2端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
	 	GPIO_Init(GPIOE, &GPIO_InitStructure); //B2上拉输入
	 	GPIO_SetBits(GPIOE,GPIO_Pin_3);        //上拉		
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;				     // F9，PF11端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	 	GPIO_Init(GPIOE, &GPIO_InitStructure); //PF9,PF11推挽输出
	 	GPIO_SetBits(GPIOE, GPIO_Pin_9);       //上拉
		
	  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;				   // F9，PF11端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	 	GPIO_Init(GPIOD, &GPIO_InitStructure); //PF9,PF11推挽输出
	 	GPIO_SetBits(GPIOD, GPIO_Pin_9);  //上拉
		
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;				 // PF10端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
	 	GPIO_Init(GPIOE, &GPIO_InitStructure);  //PF10上拉输入
	 	GPIO_SetBits(GPIOE,GPIO_Pin_10);  //上拉		
